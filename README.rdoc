...
Not giving any parameters will generate the single table model.
...
If you audit trail use a single polymorfic table as your model provide it using the <tt>:polymorphic</tt> option:

  class Model < ActiveRecord::Base
    state_machine :state, :initial => :start do
      store_audit_trail :polymorphic => true
      ...
